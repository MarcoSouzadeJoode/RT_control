...
class Ephemeris:
    ...
    def __init__(self, name, start, stop, solar_system=False, solar_system_id_type="majorbody"):
        ...
        if self.duration <= 0:
            ...
            if self.solar_system:
                ...
                self.outfilename = f"{self.name}_{self.start}.txt"
    ...
    def simbad(self):
        ...
        try:
            ...
            _ra, _dec = _dso.to_string("hmsdms").split(" ")
            print(_ra, _dec)
    ...
    def generate_output_file(self):
        # a method for generating the resulting ephemeris file.
        # will be executed only in the SSO branch.
        with open(self.outfilename, "w+") as output_file:
...
class Conversion:
    ...
    def __init__(self, RA, DEC, start, stop, name=None):
        ...
        if self.duration <= 0:
            ...
            self.DSO = SkyCoord(self.RA + self.DEC)
    ...
    def generate_output_file(self):
        ...
        output_file_name = f"{self.name}_{self.start}.txt"
...
#Quas = Conversion(Quasar.RA, Quasar.DEC, start="2021-07-13 08:40:00", stop="2021-07-13 9:00:00", name="3C273")

#DSO = Ephemeris(name="Quasar", start="2021-07-14 08:40:00", stop="2021-07-14 9:00:00", solar_system=False)
...
def handle_client(conn, addr):
    ...
    while connected:
        ...
        if msg_length:
            ...
            expected_filename = output_file_name_pregiver(msg)
            send(conn, expected_filename)
            results = message_parsing(msg)

            # In case of a smooth query, a string will be returned.
            # in case of errors, a list of errors is returned and
            # they are individually sent, in a pair of [code \n info].
            ...
            if type(results) == str:
                ...
                for res in results:
                    error_string = f"{res[0]}\n{res[1]}"
                    print("[SENDING] ", repr(error_string))
                    send(conn, error_string)

    # conn.close()

def output_file_name_pregiver(msg):
    parts = msg.split("\n")
    request_type = parts[0]
    if request_type == "resolve_request":
        _ofn = f"expected_filename\n{parts[1]}_{parts[2]}.txt"
    elif request_type == "pushing_ra_dec":
        _ofn = f"expected_filename\n{parts[5]}_{parts[3]}.txt"
    return _ofn
...
# TODO: generate filename before starting to create file -- OK
# TODO: read and write RA DEC in human readable format


    if request_type == "pushing_ra_dec":
        CNV = Conversion(RA=parts[1], DEC=parts[2], start=parts[3], stop=parts[4], name=parts[5])
